...
模板内置了中文和英文支持。你可以使用 `contest-conf` 中的 `language` 参数来切换语言：

```typst
#show: contest-conf.with(
  language: "en",  // 使用 "zh" 为中文（默认）或 "en" 为英文
  title: "程序设计竞赛",
  problems: problems
)
```
...
语言设置会影响所有 UI 元素，包括：
- 章节标题："输入格式"、"输出格式"、"样例"、"提示"
- 样例表格标题："标准输入"、"标准输出"
- 题目列表标题："试题列表"
- 扉页上的试题册信息和提醒文本

**注意：** 语言参数只影响 UI 文本。题目内容（描述、输入/输出说明等）应该在题目定义中使用你期望的语言编写。
...
**参数:**
  ...
  - `language` (str, 可选): UI 文本语言 - "zh" 为中文或 "en" 为英文（默认："zh"）
